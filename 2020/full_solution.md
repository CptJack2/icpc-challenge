...
#M Trailing Digits
有一批货物，每批有一个单价，如果将它们以n个捆绑一起销售，单价乘以n可能会得到尾数为连续数字的总售价，这样对销售有帮助。现在给出货物的单价b，一个数字d，和最大捆绑数量a，问在捆绑数量不超过a时，总售价的尾数最多能够包含多少个d。

问题其实是一个数论的问题。设总售价的形式为10^l*x+dddd...(l个)，问题可以简化为解同余方程10^l*x+dddd...(l个)≡0(mod b)，l的取值从a的长度不断倒数到0，遇到的第一个可行解即是答案。
同余方程a*x≡m(mod b)有解的充要条件是gcd(a,b)能够整除m，通过扩展欧几里得算法可以找到整数(x,y)使得a*x+b*y=gcd(a,b)，对整数(x,y)通过一些变换就可以得到同余方程的解x'。值得注意的是，对于任意整数k，x‘+k*b/gcd(a,b)也是同余方程的解。
首先求出10^i和dddd...(i个)对b的余数r1i和r2i，根据同余的性质，方程可以等价为r1i*x≡-r2i(mod b)。接下来解出方程后还需要注意一些小细节，比如x'也可能包含尾数d；比如求出x'后，可能加上b/gcd会得到一个位数相同但尾包含d的解；比如当求出x'==0时，0显然不能做开头部分，应该令x'=gcd。

#N What's Our Vector, Victor?
在一个d维的空间里有n个向量，有一个未知的向量，但知道它和那n个向量的欧几里得距离，要把这个未知的向量求出来。

这个其实是一个高维空间的计算几何问题，其实就是求高维空间里的球（距离某个固定点的距离为给定长度的点的集合）相交的交点。拿三维空间的球举例，两个球相交，形成的交集是一个圆；再用一个球和它们相交，交集是两个点，也就是一维空间距离固定点长度相等的集合；如果再多一个球，那么就可以确定唯一的点了。推广到高维空间，每多一个高维球，交集的维度就可以下降一维，只需要d+1个球就可以唯一确定一点（假设没有球心相同的球），再多就无解了，少了的话可以任取集合中的一点作为答案。
解法就是不断模拟集合交点集合降维的过程，取两个球，由于两个超维球相交的超平面垂直于球心连线，所以可以用球心连线方向上的单位向量作为一个基。然后可以求出球心在超平面上的投影，就是两个超维球相交形成的超圆的圆心。两个球的在超平面上的交点和两个球心组成了一个三角形，通过余弦定理可以求出球心到超平面的距离，然后通过勾股定理求出超平面上的超圆的半径。有了超圆的坐标，超平面的法向量，超圆的半径，就可以唯一确定这个超圆，也就相当于降了维。
对后面剩下的球也不断重复这个过程，先求出超球在超平面上的投影，再通过上面的余弦定理勾股定理对交集降维。最后，如果超平面被降成0维，那么圆心就是答案；否则在超圆上任取一点即可作为答案。

#O Which Planet is This?!
有两张星球的地图，各自都是一系列经纬度的坐标，由于星球可以旋转，所以两张地图的经度可能有偏差，但是两张地图同一点的纬度是一定一样的，现在问能不能对第二张地图的经度做一些旋转调整，使得上面的点和第一张地图的点重合，来判断它们是不是描述同一个星球的地图。

由于地图的经纬度最多有4位小数，首先把经纬度化成整数便于处理，这里要注意小数和整数的转化的陷阱。然后按照经度第一关键字，维度第二关键字给坐标排序，两张地图就变成了两个坐标序列，接下来的问题就变成了两个坐标序列能否匹配上的问题了。如果两个坐标序列中的两点要能匹配上，首先它们的维度必须要相同，其次，它们和各自序列中前一个点的相对位置也要相同，也就是说它们的前一点的纬度要相同，它们和前一点的经度差也要相同。这样，就定义了序列中点是否相等的一个数学关系，接下来就可以用一些经典的字符串比对算法来进行序列的匹配了，我使用的是AC自动机中构建fail link的方法来加速比对。由于球的性质，序列其实是环形首尾相接的，可以定地图1为待匹配的序列，用前面定义的相等关系为它构建fail link，然后从图2的头开始逐一往后匹配图1，由于不确定图2的哪一个点匹配图1的头，第一次遍历图2的每一点去尝试匹配图1的时候总能碰上，然后在后续的第二次遍历图2序列中，图1的序列必然能一直匹配到尾部，也是使用了转两圈的技巧。如果图2转了两圈能匹配上图1，那么地图是相同的，否则就是不同的。