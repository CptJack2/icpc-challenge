...
##解法
这道题是一道计算几何的题目，但求最短路径的过程又用到了图论里的dijkstra算法。首先看两个典型的路径。
![](sample1.png)
![](sample2.png)
...
# K Take on Meme
## 题目大意
每个meme有x和y两个属性。现在举行一个系列的meme选举赛，从若干个meme中选举一个胜者，对胜者的xy属性置为正，败者的xy属性置为负，生成一个新meme。每场比赛新生成的meme也可以继续参加下一个比赛。所有比赛汇总成一颗树形结构。现在问在根节点生成的meme，它的xy属性，距离原点最大的欧氏距离可以取多少。
## 解法
根节点的结果构成的可能的meme的坐标，是一个有限点集。
问题要求从这个点集中找出距离原点最远的那个点。
但随着比赛层级的增加，点集的规模呈指数上升，所以不能采用遍历每个点的方法求最大值。

求解使用一种类似贪心法的方法，初始先用一个随机向量，沿着这个随机向量的正负两个方向找到最远（距离的确定通过在随叫向量上的投影长度确定）的两个点。然后沿这两个点的垂线的正方向往外找，如果能找到更远的点，就用这个更远的点分别和两个点作连线，向垂线的方向找，直到找不出更远的点为止。
用这样的方法，可以保证结果点集的凸包上的每一个点，一定会被遍历到。
每找一次最多只需要DFS一遍树，复杂度为O(N)。因为点集规模是N^N,这种方法的复杂度是log(N^N),所以总的复杂度为O(N^2*log(N))。

DFS单遍过程：
类似动态规划，对于每个节点，将所有子节点的min和max分别加总，得到minTotal和maxTotal；对于每个节点，计算其max+min，得到其中最大和最小的maxDiff和minDiff。
要得到最远距离，就要将其中一个子节点的-1权重置为1，收益最大的当然是min max diff最大的节点。

#L Where Am I?
##题目大意
在一个dx x dy大小的方格地图上，有若干个点标记了X，从任一点出发按照顺时针螺旋发散的形式遍历格子，会得到一个'.'和'X'组成的路径，问要区分出所有dx x dy个格子平均要走多远，最长要走多远，最长的坐标是哪些
##解法
要区分出一个格子，没有必要走到遇到所有的X，只要走到当前的路径标记能够在dx x dy个格子中唯一区分自己就可以了，当走完所有的X必然可以唯一区分。现在就是要对每个格子求唯一区分它自己要走多远。
最简单的做法是对每个格子生成路径，然后逐一比较这些路径，但这样的时间和空间复杂度都是O(dx^2·dy^2).
比较好一点的做法是，X的数目是比较少的，只有最多100个，可以根据X的位置编码每个点的路径。
实现方法是，先对每个格点，找出所有的X分别距离自己多远，然后对行进距离从0开始逐渐增大，不断筛走能区分开的格点，直到所有格点都被区分。