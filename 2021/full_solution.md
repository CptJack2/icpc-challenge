...
这个解法的核心思路也是滑动窗口，不过采用了一些技巧减少了比对次数。注意到问题限定只有100种颜色，可以将每种颜色分开成一个单独图层，将每种颜色分开后，可以只用01表示这个位置有没有对应的颜色，motif的每一列可以以bitset的形式储存。在mosiac上按
并每次用motif的一列去匹配mosaic的列的部分。建立一个fail矩阵，大小和mosiac大小一致，用来表示以这个位置作为motif的左上角能否匹配，初始化置为0，在后面的过程中不断把无法匹配的位置置1，最后遍历检查，为0的位置就是匹配上的位置。然后对motif进行转置并上下左右对称反转，并按颜色分层，最后再取反，用来生成用于匹配的模板。
最后优化到O(rpxcqx100)，即最多10^8次计算
