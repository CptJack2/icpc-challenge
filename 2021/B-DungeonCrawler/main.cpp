...
//            int64_t ymx = prev == 0 ? 0 : getLongest(prev, x, skipNd[prev][0]);//不算和x距离为2^(b-1)-1的点（y的一个子节点）和y的父节点
            skipNd[x][0] = prev;
            skipPrev[x][0] = x;
            skipDist[x][0] = d;
...
//                    fprintf(stderr,"y:%d skipPrev[x][b-1]:%d skipNd[y][0]:%d\n",y,skipPrev[x][b-1], skipNd[y][0]);
    ...
    auto anc = [&](int x, int y) { //ancestor, 找从x出发到y，经过的x的祖先
        ...
        while (depth[y] > depth[x]) {
            for (int b = skipNd[y].size() - 1; b >= 0; b--)
        ...
        while (depth[x] > depth[y]) {
            for (int b = skipNd[x].size() - 1; b >= 0; b--)
